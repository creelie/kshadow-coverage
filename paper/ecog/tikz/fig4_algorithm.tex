...
\node[dec, below=of dist] (floor) {$\rho_C<r$\,?\\[1pt]{\scriptsize site floor}};
\node[key, below=0.7cm of floor] (greedy) {\textbf{Farthest-point step.} Add the
  site of $C$ farthest from the contacts chosen so far; update
  $\rho_1(P_n),\dots,\rho_k(P_n)$ exactly.};
...
\node[stop, right=1.2cm of floor] (nofloor) {\textbf{Infeasible for these sites.}
  No array on $C$ covers $X$ at radius $r$, at any contact count: enlarge $C$
  (depth leads, sulcal strips) or $r$.};
...
\draw[flow, dashed, cInk2] (nofloor.south) |- node[lab, pos=0.7, above] {enlarge $C$}
  ($(greedy.east)+(0,0.25)$);
% step numbers
